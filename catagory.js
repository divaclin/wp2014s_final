	...
	query.find({
	  success: function(data) {
		  for(var i=0;i<data.length;i++){
              ...
              $("#catagory_content").append('<div class="issue-block" style="width:'+window.screen.width/4+'px;"><div class="issue-content"><p>'+data[i].toJSON().title+'</p><img src="'+url+'" class="issue-img" /><div>'+data[i].toJSON().creatAt+'</div><span><a href="#">我要連署</a><a href="#">連署聲明</a></span></div></div>');  
		  }
	  }
	});
}